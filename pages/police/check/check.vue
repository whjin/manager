        ...
        <scroll-view scroll-y="true" class="uni-flex-item uni-flex uni-column" style="height: 100%; width: 100%">
          ...
          <view class="primary-content">
            ...
            <ld-select ref="ldSelect" :multiple="true" :list="checkContentList" label-key="value" value-key="code"
              :placeholder="checkContentList.length ? '请选择检查内容' : '请先选择检查类型'
          ...
          <view class="primary-content violate-goods">
            ...
            <view class="upload-img-box">
              <view class="goods" v-for="(item, index) in addImgList" :key="item.uri">
                ...
                <common-icons iconType="icondelete" size="35" color="#fff" @click="deleteImg(index)" :iconStyle="{
                  position: 'absolute',
                  top: '5px',
                  right: '5px',
                }" />
        ...
        <scroll-view v-else scroll-y="true" style="height: 347upx; width: 100%" @scrolltolower="handleSearchDataToLower">
          <view class="uni-flex table-td-divider" v-for="(item, index) in checkRecordList" :key="index">
            <view class="uni-flex-item table-td">
              {{ saveCheckParams.roomName }}</view>
            <view class="uni-flex-item table-td">{{ item.checkType }}</view>
            <view class="uni-flex-item table-td">
              {{ item.checkStartTime }}</view>
            <view class="uni-flex-item table-td">
              {{ item.checkContentValue }}</view>
            <view class="uni-flex-item table-td">{{
...
export default {
  ...
  computed: {
    ...
    // 检查内容列表
    checkContentList() {
      let curContentList = this.checkTypeList.find(
        (item) => item.code === this.saveCheckParams.inspectionType
      );
      if (curContentList) {
  ...
  methods: {
    ...
    async getCheckTypeAndContent() {
      let res = await Api.apiCall(
        "get",
        Api.police.check.getCheckTypeAndContent,
        null,
        true
      );
      if (res.state.code === 200) {
        ...
        this.saveCheckParams.registerTime = getDate(
          this.nowTimestamp,
          "minutes"
        );
      }
    ...
    addImg() {
      let self = this;
      getApp().globalData.crashHandle.startGuard(0);
      uni.chooseImage({
        count: 6,
        sizeType: ["original"],
        sourceType: ["camera"],
        ...
        complete: function () {
          getApp().globalData.crashHandle.startGuard(10000);
    ...
    async getCheckRecord() {
      ...
      let res = await Api.apiCall(
        "post",
        Api.police.check.getCheckRecord,
        params,
        true
      );
      /**
