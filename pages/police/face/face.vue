			...
			<div class="face-content">
				...
				<div class="face-box">
					<div class="face-menu-list">
						...
						<div class="face-menu-item" :class="currentPage == 3 ? 'face-menu-active' : ''"
							@click="switchPage(3)">
							<common-icons iconType="iconrecord" size="46" color="#fff" />
							<text>发起点名</text>
						</div>
					...
					<div v-if="currentPage == 1" class="face-register-container">
						<div class="face-form-content">
							<div class="face-form-box">
								...
								<div class="face-select">
									<label>面对面民警：</label>
									<input type="text" class="input-img" v-model="personInfo.name" disabled />
								</div>
								<div class="face-select">
									<label>登记时间：</label>
								...
								<div class="face-select">
									...
									<xfl-select :list="faceTypeList" clearable :placeholder="'请选择面对面类型'"
										@change="selectFaceType" @clear="clearFaceType"></xfl-select>
								</div>
						...
						<div class="face-button-box">
							<div class="face-button button-cancel" @click="cancelFaceReg">取消登记</div>
							<div class="face-button button-save" @click="saveFaceRegister">保存登记</div>
						</div>
					</div>
					<div v-if="currentPage == 2" class="face-record-container">
						<div class="record-form-content">
							<div class="face-form-box">
								...
								<div class="face-select">
									<label>登记时间：</label>
									<e-picker mode="date" class="picker-img" @change="selectSearchDate"
										:placeholder="'请选择登记时间'">
							...
							<div class="face-button-box">
								<div class="face-button button-query" @click="queryFaceRecord">查询</div>
							</div>
						</div>
						<div class="record-table-box">
							...
							<scroll-view scroll-y="true" class="record-table-scroll">
								<div class="table-content" v-for="(item, index) in faceRecordList" :key="index">
									...
									<div class="record-table-item" style="flex: 1" @click="getFaceDetailInfo(item)">查看详情
									</div>
								</div>
							</scroll-view>
						</div>
				...
				<div class="face-modal-box">
					...
					<div class="face-modal-content">
						<div class="face-detail-box">
							...
							<div class="face-detail">
								<label>面对面民警：</label>
								<input type="text" class="input-img" v-model="faceDetailInfo.accountName" disabled />
							</div>
							<div class="face-detail">
...
export default {
	...
	computed: {
		...
		// 监室信息
		roomInfo() {
			const { roomNo, roomName } = uni.getStorageSync("managerInfo");
	...
	methods: {
		...
		async getPoliceInfo() {
			...
			let res = await Api.apiCall("get", Api.police.face.getRoomPoliceInfo + `?roomNo=${roomNo}`, null);
			if (res.state.code == 200) {
				this.policeList = res.data;
				this.policeList.map(item => {
					if (this.personInfo.code == item.code) {
						this.policeInfo.code = item.code;
						this.policeInfo.value = item.value;
					}
				});
			}
		},
		// 获取面对面类型
		async getFaceType() {
			let res = await Api.apiCall("get", Api.police.face.getFaceType, null);
			if (res.state.code == 200) {
		...
		// 选择面对面类型
		selectFaceType(e) {
			this.faceType = e.originItem.code;
		},
		// 清空面对面类型
		clearFaceType() {
			this.faceType = "";
		...
		async saveFaceRegister() {
			...
			let res = await Api.apiCall("post", Api.police.face.saveFaceRegister, params);
			if (res.state.code == 200) {
