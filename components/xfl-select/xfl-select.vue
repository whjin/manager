<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		...
		<input  
			...
			@focus="onFocus" @blur="onBlur" @input="onInput" @confirm="$emit('confirm', $event)" :style="'width:' + widthStyle + 'upx;'"
		>
		<!-- 显示框 -->
		...
		:style="'top:' + listTop__ + 'px;width:' + widthStyle + 'upx;'" v-show="isShowList">
			<span class="popper__arrow"></span> <!-- 列表框左上角的空心小三角 -->
			<scroll-view 
				class="list" style="background-color: rgba(42,66,115,0.95);"  
...
<style scoped lang="less">
    ...
    position: relative;
		...
		.right-arrow{
			...
			position: absolute; font-size: 1em; right: 0upx; display: flex; 
			top: 0;
		...
		.clear{
			...
			background-color: @hover-color; border-radius: 50%;  padding: 1.38upx;
		}
		
		/****** 列表框部分样式 *****/
		.list-container{
			position: absolute; width: 100%; left: 0; top: 34.72upx;
			...
			.popper__arrow{
				 ...
				 filter: drop-shadow(0 1.38upx 8.33upx rgba(0,0,0,.03));
				 left: 30%;
			...
			.list{
				...
				box-shadow: 0 1.38upx 8.33upx 0 rgba(0,0,0,.1);
				padding: 3.47upx 0;
